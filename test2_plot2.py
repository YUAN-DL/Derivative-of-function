...
F=-np.cos(L)*np.exp(np.cos(L))+np.sin(L)*np.sin(L)*np.exp(np.cos(L))
plt.plot(L, F,linewidth=0.5,color='b')
x1=[]
y1=[]
with open('Test2_2_Finite_difference.txt', 'r') as f:
    data = f.readlines()
    for line in data:
        odom = line.split(' ')
...
x2=[]
y2=[]
with open('Test2_2_Fourier_spectra.txt', 'r') as f:
    data = f.readlines()
    for line in data:
        odom = line.split(' ')
